<?php 
  ...
  WHERE INDICE = '".$numAtendimento."'"; 

  $res = mssql_query($sql);

  $sql_atendimento = "SELECT COUNT(INDICE) AS 'Nº Atendimento' 
  FROM [CUSTOMER_NCM].[dbo].[HOTLINE_STATUS_TABULADOS_TESTE] 
  WHERE INDICE = '".$numAtendimento."' AND 
  STATUS_ATENDIMENTO IS NOT NULL";

  $atendimentos_encontrados = mssql_query($sql_atendimento);

  $sql_status = "SELECT STATUS_ATENDIMENTO AS 'Atendimento', DETALHE_ATENDIMENTO AS 'Detalhe Atendimento' 
  FROM [CUSTOMER_NCM].[dbo].[HOTLINE_STATUS_TABULADOS_TESTE] 
  WHERE INDICE = '".$numAtendimento."' AND 
  STATUS_ATENDIMENTO IS NOT NULL";

  $status_atendimentos = mssql_query($sql_status);
  ...
  else
  {
    ...
    $indice= $dados['INDICE'];	
    $canVendas = $dados['cb_Canal_Vendas'];
    $nomVendedor = $dados['tx_nome'];
    $logVendedor = $dados['tx_login'];
    $cpfCnpj = $dados['tx_CPF_CNPJ'];
    $opcContratoProposta = $dados['cb_contrato_proposta'];
    $conProposta = $dados['tx_contrato'];
    $ocoCriada = $dados['tx_ocorrencia_criada'];
    $modAtendimento = $dados['cb_Modalidade_Atendimento'];
    $estado = $dados['cb_Estado'];
    $cidade = $dados['cb_cidade'];
    $lojVendedor = $dados['tx_loja_vendedor'];    
  }
  $nome = $_SESSION['nome']." ".$_SESSION['sobrenome'];
...
<head>
    ...
    <link rel="stylesheet" href="bootstrap/v3.4.1/css/bootstrap.min.css">
  ...
  <script>
    ...
    function submitdata()
    {
      var id= '1000';
      var nome= 'teste';      
      var staAtendimento = document.getElementById('staAtendimento').options[document.getElementById('staAtendimento').selectedIndex].text;
      var detAtendimento = document.getElementById('detAtendimento').options[document.getElementById('detAtendimento').selectedIndex].text;
      var numAtendimento = document.getElementById('numAtendimento').value;

      var msg = "";

      if(staAtendimento == "" || staAtendimento == "0" || staAtendimento == "SELECIONE:")
      {
        msg += "Selecione o campo 'Defina os atendimentos realizados nesta ligação:'!\n";
      }

      if(detAtendimento == "" || detAtendimento == "0" || detAtendimento == "SELECIONE:")
      {
        msg += "Selecione o campo 'Detalhe do Atendimento:'!\n";
      }
      ...
      else
      {
          ...
          $.ajax({
          type: 'POST',
          url: 'salvar_atendimentos.php',
          data: {
          idOperador:id, nomeOperador:nome, status:staAtendimento, detalhe:detAtendimento, atendimento:numAtendimento
          },
          dataType: "html",
  ...
  <body class="bg-light" onload="habilitaCampos();">
    <form name="finalizarHuawei" id="finalizarHuawei" method="post" action="" onKeyUp= "" enctype="multipart/form-data" onsubmit= "submitdata();">
      <div class="container">
            ...
            <form class="needs-validation" novalidate>
              ...
              <div class="row">
                <div class="col-md-1 mb-2">
                  <button class="btn btn-primary btn-sm" type="submit" onclick=""><span class="glyphicon glyphicon-plus"></span></button>
                  
                </div>
                <div class="col-md-1 mb-2">
                  <button class="btn btn-primary btn-sm" type="" onclick=""><span class="glyphicon glyphicon-minus"></span></button>
              ...
              <div class="row">
                <div class="col-md-3 mb-2">
                  <table class="table table-bordered table-sm">
                    ...
                    <tbody >
                        <?php 	                  
                        while ($row = mssql_fetch_array($atendimentos_encontrados))
                        {
                          
                                                                                    
                          echo "
                          <tr>                          
                          <td>".utf8_encode($row['Nº Atendimento'])."</td>
                          </tr>								
                          ";								
                        }	                  
                        ?>
                ...
                <div class="col-md-9 mb-2">
                  <table class="table table-bordered table-sm">
                    <thead>
                      <tr>
                        <th>Atendimento:</th>
                        <th>Detalhe Atendimento:</th>
                      </tr>
                    </thead>
                    <tbody >
                        <?php 	                  
                        while ($row = mssql_fetch_array($status_atendimentos))
                        {                                                                                                             
                          echo "
                          <tr>                          
                            <td>".utf8_encode($row['Atendimento'])."</td>
                            <td>".utf8_encode($row['Detalhe Atendimento'])."</td>
                          </tr>								
                          ";								
                        }	                  
                        ?>
